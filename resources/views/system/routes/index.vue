  ...
  methods: {
    ...
    fetchData () {
      return permissions().then(res => {
        const data = utils.responseFormat.string2number(res, ['id', 'parent_id', 'client', 'level', 'order', 'type'], true).filter(item => item.type === 1)
    ...
    update (node, data, newData, operateType, isDrag) {
    ...
    submit (data) {
      ...
      return combine(list.map(item => ({ ...item, type: 1 })))
    },
    updateDragData (parent) {
      ...
      const params = result.filter(item => {
        const origin = this.originData.find(data => data.id === item.id)
        if (origin) return origin.order !== item.order || origin.parent_id !== item.parent_id
        return true
      })
      this.submit(params).then(() => {
